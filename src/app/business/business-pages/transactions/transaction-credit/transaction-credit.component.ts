...
@Component({
  ...
  styleUrls: ['./transaction-credit.component.scss']
})
export class TransactionCreditComponent implements OnInit {
  ...
  constructor(private dialog:MatDialog,private service:ApiServicesService,private router:Router) { }
  ...
  transactionsList(){
    ...
    this.service.get(type, API_ROUTES.Transactions.transactionList).pipe().subscribe((res) => {
      ...
      if(res.success) {
        ...
        this.data = res.result.filter((element:any) => { if (element.transaction_type.toLowerCase() == 'credit') return element});
      }
  ...
  showdropdown(serial: any){
    ...
    this.cards.map(value => {
      if(value.serial == serial) {
        // console.log("aaaaaaaa"+value.serial);
  ...
  payment(id:any){
    this.router.navigate(['/business/transactions/add-new-transaction'],{queryParams: {id:id,status:true}});
  }
